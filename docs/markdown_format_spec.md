...
4. **Hover 预览**：鼠标悬停在 cite 数字按钮上约 200ms 后，显示来源预览弹窗：
   - 展示来源 **标题**（` — ` 前的正文，或整行摘要）
   - 若有 **简介**（` — ` 与末尾链接之间）一并展示
   - 展示可点击的 **外链**（行尾 ` - https://...` 或行内 Markdown 链接）
   - 鼠标可移入弹窗点击链接（走系统浏览器）
   - 鼠标离开 cite 按钮，或进入弹窗后再离开，弹窗关闭
   - **点击 cite 按钮**仍执行第 1 步跳转，与 hover 互不干扰
...
任务复选框：单元格内容为 `- [ ]` / `- [x]`（或 `*`、`+` 前缀）或 `[ ]` / `[x]` 时，渲染为可点击复选框并写回源文件；同一单元格后还可跟正文（如 `- [ ] 子任务说明`）。
