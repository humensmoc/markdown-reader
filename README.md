...
- **批注**：选中正文后添加批注；AI 修改正文后，回应批注的主要改动会直接在正文中高亮，批注卡片只显示问题和回复；解决后的批注继续留在 Markdown 文末，并可从右下角“批注”按钮查看
- **链接**：`http` / `https` / `mailto` 走系统浏览器；相对 `.md` 在编辑器内打开；`#anchor` 在 webview 内跳转；危险协议（如 `javascript:`）会被拦截

#### AI 批注回写格式

Markdown Reader 不绑定某一家 AI。AI 或自动化工具修改正文后，应保留批注块，并在修改前后对照确认实际改动。在元数据中写入修改后正文里可精确匹配的 `change_quote`：它必须来自本次实际新增或改写的内容，并指向最能代表该批注解决结果的主要改动，而不是文档中最先出现的改动、未变化的上下文或顺手调整的次要内容。有多处改动时只记录一处，并尽量带上足够上下文以便唯一定位。改动简述追加在 `AI 回复` 下：
...
status: "open"
change_quote: "最能代表本批注解决结果的实际改动正文"
-->
> **批注：用户最初选中的内容**
>
> 用户提出的问题或修改要求
...
阅读器会把正文高亮从原选区切换到 `change_quote`，并兼容表格行、列表、标题和常见行内 Markdown 语法。点击“解决”后会写入 `status: "resolved"` 和 `resolved_at`；批注块不删除，也不会作为普通正文渲染。
